...
const generateAccessToken = async (refresh_token) => {
  ...
  const refresh_token_decoded = await jwt.verify(
    refresh_token,
    process.env.REFRESH_TOKEN_SECRET,
  );
  // Save refresh token to database
...
const checkIsAccessTokenValid = async (access_token, user) => {
  ...
  let access_decoded = jwt.verify(
    access_token,
    process.env.ACCESS_TOKEN_SECRET,
  );
  let refresh_decrypted = await decryptToken(user.refresh_token);
  let refresh_decoded = jwt.verify(
    refresh_decrypted,
    process.env.REFRESH_TOKEN_SECRET,
  );
...
const createVerificationToken = async (user) => {
  ...
  const verification_token = await jwt.sign(
    payload,
...
const verifyVerificationToken = async (token) => {
  // Verify token
  const token_decoded = await jwt.verify(
    token,
    process.env.VERIFICATION_TOKEN_SECRET,
  );
